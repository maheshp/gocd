...
define(['lodash', "pipeline_configs/models/tasks", "string-plus"], function (_, Tasks, s) {
  describe("Task Model", function () {
    ...
    describe("Exec", function () {
      ...
      describe('Toggle args', function () {
        it('should toggle args as list to string', function () {
          ...
          expect(task.args().data()).toEqual('-c ssh');
        });

        it('should toggle args as string to list', function () {
          var task = new Tasks.Task.Exec({
            arguments: "-c 'ssh somepath'"
          });

          task.toggleArgs();

          expect(task.args().data()).toEqual(['-c', 'ssh somepath']);
        });
      });
    });
